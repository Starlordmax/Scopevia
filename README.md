...
| [docs/76-measurements-draw-validation-visible-fix.md](docs/76-measurements-draw-validation-visible-fix.md) | Fix: Measurements' Draw layout validation wasn't actually visible — Save was `disabled` on the exact conditions it should report, and native `min`/`max`/`type="email"` constraints silently blocked submission app-wide; adds `noValidate` to every validated form |
| [docs/77-ai-proposal-text-generation.md](docs/77-ai-proposal-text-generation.md) | Feature: AI writing assistant on Terms & Pricing (draft Terms/Exclusions/Notes for client via OpenRouter, server-only, with a local fallback template when unconfigured) — architecture, prompt design, UX, permissions, rate limiting |
| [docs/78-business-profile-ai-context.md](docs/78-business-profile-ai-context.md) | Tenant-level Business profile (Profile → Business profile) feeding the AI writing assistant — fields, validation, permission reuse |
| [docs/79-openrouter-security.md](docs/79-openrouter-security.md) | Security model for the OpenRouter integration — API key handling, data minimization, tenant isolation, rate limiting, testing without calling the real API |
