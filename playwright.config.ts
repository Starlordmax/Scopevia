...
export default defineConfig({
  ...
  projects: [
    { name: "setup", testMatch: /.*\.setup\.ts/ },
    {
      ...
      dependencies: ["setup"],
      // pipeline.mobile.spec.ts asserts real mobile-viewport layout (stacked
      // columns, no horizontal scroll) — those assertions are only true
      // under the "mobile" project's 390x844 viewport, so it must not also
      // run here under a desktop viewport.
      testIgnore: [/.*\.setup\.ts/, /pipeline\.mobile\.spec\.ts/],
    },
    {
      name: "mobile",
      ...
      dependencies: ["setup"],
      testMatch: /pipeline\.mobile\.spec\.ts/,
    },
  ],
  webServer: {
