...
test.describe("Measurements / Takeoff builder (mobile, 390x844)", () => {
  test("manual entry, draw layout, and generate material all work with no horizontal overflow", async ({ page }) => {
    ...
    // Draw layout: drag a rectangle via mouse-emulated pointer events (the
    // canvas listens for Pointer Events, which Playwright's mouse API
    // dispatches regardless of device — the same code path a real touch
    // drag exercises).
    ...
    await measurementsPanel.locator("#drawGroupId").selectOption({ label: "Bedroom" });
    await measurementsPanel.locator("#drawName").fill("Bedroom sketch");
